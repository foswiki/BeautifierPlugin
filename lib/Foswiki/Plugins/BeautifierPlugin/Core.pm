package Foswiki::Plugins::BeautifierPlugin::Core;
...
sub highlight_text {
    ...
    $self->{context} = new Foswiki::Plugins::BeautifierPlugin::Context();
    $self->{context}
      ->from_language( $self->{highlightfile}, $self->{output_module} );
    $self->{langstack}    = ();
    $self->{contextstack} = ();
    ...
    for ( my $i = 0 ; $i < $numlines ; $i++ ) {
        ...
        my $line = $lines[$i];
        $line =~ s/\r$//;
        if ( $self->{context}->{preprolength} > 0
            && substr( $line, 0, $self->{context}->{preprolength} ) ==
            $self->{highlightfile}->{prepro} )
        {
        ...
        if (   defined( $self->{highlightfile}->{lineselect} )
            && !( $self->{context}->{inselection} )
            && substr( $line, 0,
                length( $self->{highlightfile}->{lineselect} ) ) ==
            $self->{highlightfile}->{lineselect} )
        {
            $out .= $self->{context}->{select_parts}[0];
            $line =
              substr( $line, length( $self->{highlightfile}->{lineselect} ) );
        ...
        my $sw =
          $self->starts_with( $lineorig,
            @{ $self->{highlightfile}->{unindent} } );
        ...
        for ( my $j = 0 ; $j < $ln ; $j++ ) {
            ...
            if (
                   defined( $self->{highlightfile}->{selecton} )
                && !$self->{context}->{inselection}
                && !$self->{context}->{inquote}
                && !$self->{context}->{inbcomment}
                && substr( $line, $j,
                    length( $self->{highlightfile}->{selecton} ) ) eq
                $self->{highlightfile}->{selecton}
              )
            {
                if ($DEBUG) { print "Selection open\n" }
                $lineout =
                  $self->munge($lineout) . $self->{context}->{select_parts}[0];
                $out .= $lineout;
                $lineout = "";
            ...
            if (
                   defined( $self->{highlightfile}->{selectoff} )
                && $self->{context}->{inselection}
                && substr(
                    $line, $j,
                    length( $self->{highlightfile}->{selectoff} )
                ) eq $self->{highlightfile}->{selectoff}
              )
            {
            ...
            if (   !$self->{context}->{lineselect}
                ...
                && !$self->{context}->{inquote}
                && !$self->{context}->{inbcomment}
                && in_array( $currchar, $self->{context}->{startingbkonchars} )
              )
            {
                ...
                if ( $currmax != 0 ) {
                    ...
                    $self->{context}->{closingstrings} =
                      @{ $self->{context}->{bcomatches} }{$bkc};
                    ...
                    $out .= $lineout;
                    $out .=
                      @{ $self->{context}->{blockcomment_parts} }[0] . $bkc;
                    $lineout = "";
...
# Line comments. Once we get a line comment we can skip straight to the next line, as nothing else can be done.
            if (
                   !$self->{context}->{lineselect}
                && !$self->{context}->{inselection}
                && !$self->{context}->{inquote}
                && !$self->{context}->{incomment}
                && !(
                       $self->{highlightfile}->{perl}
                    && $j > 0
                    && substr( $line, $j - 1, 1 ) eq "\$"
                )
              )
            {
                my $currmax = 0;
                ...
                && $self->{context}->{inbcomment} )
            {
                ...
                if ( $currmax != 0 ) {
                    ...
                    $lineout .=
                      $bku . $self->{context}->{blockcomment_parts}[1];
                    $out .= $lineout;
                    $lineout = "";
            ...
            my $delim = (
                     $j == 0 
                  || in_array( $currchar, $self->{context}->{alldelims} )
                  || (
                    $j > 0
                    && in_array(
                        substr( $lineorig, $j - 1, 1 ),
                        $self->{context}->{alldelims}
                    )
                  )
            );
            ...
            if (
                   !$self->{context}->{lineselect}
                && !$self->{context}->{inselection}
                && !$self->{context}->{escaping}
                && (
                    (
                        in_array( $currchar,
                            $self->{highlightfile}->{stringchars} )
                        && $self->{context}->{inquote}
                        && $currchar eq $self->{context}->{currquotechar}
                    )
                    || (
                        in_array( $currchar,
                            $self->{highlightfile}->{stringchars} )
              ...
              )
            {
                ...
                # First quote.
                if (   !$self->{context}->{escaping}
                    && defined( $self->{context}->{inquote} )
                    && !$self->{context}->{inquote} )
                {
                    $lineout = $self->munge($lineout);
                    $out .= $lineout;
                    ...
                    if ( defined( $self->{statobj} )
                        && $self->{statobj}->{harvest_strings} )
                    {
                        $self->{string_cache} .= " ";
                    }
                    if ( $self->{context}->{prepro} ) {
                        $lineout =
                            $self->{context}->{prepro_parts}[1]
                          . $currchar
                          . $self->{context}->{quote_parts}[0];
                    }
                    else {
                    ...
                    && $currchar eq $self->{context}->{currquotechar} )
                {
                    ...
                    if ( $self->{context}->{prepro} ) {
                        $lineout .=
                            $self->{context}->{quote_parts}[1]
                          . $self->{context}->{prepro_parts}[0]
                          . substr( $lineorig, $j, 1 );
                    }
                    else {
                        $lineout .=
       ...
       # If we've got an indent character, increase the level and add an indent.
                ...
                && !$self->{context}->{inquote}
                && (
                    my $stri = $self->starts_with(
                        substr( $line, $j ),
                        @{ $self->{highlightfile}->{indent} }
                    )
                ) ne ""
              )
            {
            ...
            elsif ( !$self->{context}->{inwhitespace}
                || $currchar ne " "
                || $currchar ne "\t" )
            {
                if ($DEBUG) { print "String checks\nAdding $currchar..."; }
                if (   $self->{context}->{inquote}
                    && defined( $self->{statobj} )
                    && $self->{statobj}->{harvest_strings} )
                {
            ...
            if ( $self->{context}->{inquote} && $self->{context}->{escaping} ) {
                ...
                && $currchar eq $self->{highlightfile}->{escchar}
                && !$self->{context}->{escaping} )
            {
...
sub starts_with {
    ...
    foreach (@array) {
        ...
        if (
            (
                (
                    !$self->{highlightfile}->{nocase}
                    && substr( $text, 0, $l ) eq $_
                )
                || ( $self->{highlightfile}->{nocase}
                    && lc( substr( $text, 0, $l ) ) eq lc($_) )
            )
            && (   substr( $text, $l, 1 ) eq " "
                || $l == 1
                || @{$text}[$l] eq "\n"
...
sub munge {
    ...
    if ( $self->{context}->{inselection} || $self->{context}->{lineselect} ) {
        return $munge;
    }
    if ( !$self->{context}->{prepro} ) {
        for ( my $i = 0 ; $i <= $lngth ; $i++ ) {
            ...
            my $delim =
              in_array( $currchar, $self->{highlightfile}->{delimiters} );
            if ( $delim || $i == $lngth ) {
                if ($inword) {
                    $inword    = 0;
                    $oldword   = $currword;
                    ...
                    if ( defined( $self->{context}->{validkeys}{$checkword} ) )
                    {
                        if ( $self->{highlightfile}->{nocase} ) {
                            $checkword =
                              $self->{context}->{validkeys}{$checkword};
                        }
                        my $category =
                          $self->{highlightfile}->{keywords}{$checkword};

                        # [LEAF] made changes to get linkscripts working.
                        #        very rough, me bad perl hacker.
                        my $ls =
                          $self->{highlightfile}->{linkscripts}{$category};
                        my $outchunk = "";
                        if ( $ls =~ /donothing/ ) {
                            $outchunk =
                                $self->{context}->{category_parts}{$category}[0]
                              . $currword
                              . $self->{context}
                            ...
                            $outchunk =
                                $self->{context}->{category_parts}{$category}[0]
                              . $self->{highlightfile}->$ls($currword)
                              . $self->{context}
                              ->{category_parts}{$category}[1];
                        }
                        $strout .= $outchunk;
